...
surv_sads=np.atleast_1d(np.asarray(surv_sads))
...
for key in max_conn_dict.keys():
	if(len(max_conn_dict[key])!=0):
		if(key[0] in max_no_mult_cn.keys()):
			max_no_mult_cn[key[0]]+=1
		else:
			max_no_mult_cn[key[0]]=1
		if(key[1] in max_no_mult_cn.keys()):
			max_no_mult_cn[key[1]]+=1
		else:
			max_no_mult_cn[key[1]]=1
...
for i in range(len(cps_max)):
	...
	curr_max_data.append(max_no_mult_cn[max_id])
